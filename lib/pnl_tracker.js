...
async function _markDryRunTrade(trade, now) {
  ...
  const deployMs = Number.isFinite(new Date(trade.deploy_time).getTime())
    ? new Date(trade.deploy_time).getTime()
    : lastMarkMs;
  const elapsedMinutes = Math.max(0, (now - lastMarkMs) / 60000);
  const ageMinutes = Math.max(0, (now - deployMs) / 60000);
  // 🔴 BUG FIX: Jangan pakai detail?.fee_active_tvl_ratio dari API — API bisa return
  // nilai absurd (9.4 juta) untuk pool tertentu. Pakai trade.fee_tvl_ratio (stored dari
  // waktu deploy) sebagai primary. Cap max 5.0 (500% timeframe — sudah sangat generous).
  const rawFeeRatio = _num(trade.fee_tvl_ratio, 0);
  const feeRatio = Math.min(rawFeeRatio, 5.0);
  ...
  const rawFeeSolDelta = amountSol * feeRatio * (elapsedMinutes / _timeframeMinutes(timeframe)) * feeCaptureFactor;
  // Cap simulated fee accrual by elapsed wall-clock time. A high fee/TVL pool can be
  // attractive, but it should never mint a 50% paper gain on the first 30s poll.
  const maxFeePctPerHour = Math.max(0, _num(config.management?.dryRunMaxFeePctPerHour, 3)) / 100;
  const maxFeePerMark = amountSol * maxFeePctPerHour * (elapsedMinutes / 60);
  const feeSolDelta = Math.min(rawFeeSolDelta, maxFeePerMark);
  const maxFeeTotal = amountSol * maxFeePctPerHour * (ageMinutes / 60);
  const previousPaperFeeSol = _num(trade.paper_fee_sol, 0);
  const paperFeeSol = _round(Math.min(
    maxFeeTotal,
    previousPaperFeeSol + Math.max(0, feeSolDelta)
  ), 9);
  // 🔴 BUG FIX: Cap price PnL per mark ke max ±100% dari posisi.
  const rawPricePnlSol = amountSol * (pricePnlPct / 100);
  const maxPricePnlPerMark = amountSol * 1.0;
  const pricePnlSol = _clamp(rawPricePnlSol, -maxPricePnlPerMark, maxPricePnlPerMark);
  const rawPnlSol = pricePnlSol + paperFeeSol;
  // Final sanity check for open paper PnL.
  const maxPnlPct = Math.max(0, _num(config.management?.dryRunMaxUnrealizedPnlPct, 25)) / 100;
  const maxPnlSol = amountSol * maxPnlPct;
  const minPnlSol = -amountSol * 0.99;
  const pnlSol = _round(_clamp(rawPnlSol, minPnlSol, maxPnlSol), 6);
  const pnlPct = amountSol > 0 ? _round((pnlSol / amountSol) * 100, 2) : 0;
  const feeWasClamped = previousPaperFeeSol > maxFeeTotal && previousPaperFeeSol > amountSol * 0.001;
  const priorPeakPct = _num(trade.peak_pnl_pct, pnlPct);
  const peakPnlPct = feeWasClamped
    ? pnlPct
    : Math.max(Math.min(priorPeakPct, maxPnlPct * 100), pnlPct);
  ...
  return {
    ...
    fields: {
      ...
      peak_pnl_pct: _round(peakPnlPct, 2),
...
function _paperCloseReason(trade, ageMinutes, pnlPct, maxMinutes) {
  ...
  const minHoldMinutes = Math.max(0, _num(m.dryRunMinHoldMinutes, 30));
  if (ageMinutes < minHoldMinutes) return null;
